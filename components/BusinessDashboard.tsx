...
// Properly type the dynamic component props later if needed, mostly 'any' for now to match usage
...
export default function BusinessDashboard({ initialData, showStats = true }: BusinessDashboardProps) {
  const [viewMode, setViewMode] = useState<'grid' | 'table' | 'map'>('grid');
  const router = useRouter();
  const searchParams = useSearchParams();

  const page = Number(initialData.page) || 1;
