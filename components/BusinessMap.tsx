...
export default function BusinessMap({ businesses }: BusinessMapProps) {
  ...
  const center: LatLngExpression = businesses.length > 0 && businesses[0].location?.coordinates && businesses[0].location.coordinates.length === 2
    ? [businesses[0].location.coordinates[1], businesses[0].location.coordinates[0]] // GeoJSON is [lng, lat], Leaflet is [lat, lng]
    : defaultCenter;
      ...
      <MapContainer center={center} zoom={13} scrollWheelZoom={false} className="h-full w-full">
        ...
        {businesses.map((business) => {
             if (business.location && business.location.coordinates && business.location.coordinates.length === 2 && business.location.coordinates[0] !== 0) {
                 ...
                 return (
                    <Marker key={business._id} position={[lat, lng] as LatLngExpression}>
